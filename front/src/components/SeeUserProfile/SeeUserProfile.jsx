...
const SeeUserProfile = () => {
  ...
  const handleDelete = async () => {
    try {
      getSignedInUserProfile();
      if (!signedOut) {
        await axios.delete(`http://localhost:8000/user/profile/delete/${id}`, {
          withCredentials: true,
        });
        setDeleted(true);
      }
        ...
        <div>
          ...
          <div className="signup">
            <form>
              ...
              <div className="blog-btn btn-adjust">
                <NavLink exact to={`/user/edit-profile/${id}`}>
                  <button className="btn btn-edit">Edit</button>
                </NavLink>

                <NavLink exact to={`/user/edit-password/${id}`}>
                  <button className="btn btn-create" style={{marginLeft:5}}>Change Password</button>
                </NavLink>
